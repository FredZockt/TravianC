	...
	function calculateBattle($Attacker,$Defender,$def_wall,$att_tribe,$def_tribe,$residence,$attpop,$defpop,$type,$def_ab,$att_ab,$tblevel,$stonemason,$walllevel) {
		...
		if ($units['Def_unit']['hero']>0){
            ...
            if ($hero_health<=$damage_health or $damage_health>90){
				...
				mysqli_query($database->connection,"UPDATE " . TB_PREFIX . "units SET `hero` = 0 WHERE `vref`='".$defenderhero['wref']."'");
            }else{
			...
			foreach($DefendersAll as $defenders) {
				if($defenders['hero']>0) {
					...
					if ($hero_health<=$damage_health or $damage_health>90){
						...
						$database->modifyHero2('health', 0, $hero_id, 0);
						$database->modifyHero2('experience', $result[2], $hero_id, 1);
						mysqli_query($database->connection,"UPDATE " . TB_PREFIX . "units SET `hero` = 0 WHERE `vref`='".$defhero[$fromvillage]['wref']."'");
					}else{
