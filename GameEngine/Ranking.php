<?php
        ...
        class Ranking {
			...
			public function getAllianceRank($id) {
				...
				while(1) {
					if(count($database->rankarray) > 1) {
						$key = key($database->rankarray);
						if($database->rankarray[$key]["id"] == $id) {
							return $key;
							break;
						} else {
							if(!next($database->rankarray)) {
								return false;
								break;
							}
						}
			...
			public function procUsersRanking($limit="") {
				...
				return mysqli_query($database->connection, $q);
			}
