...
class Session {
    ...
    public function Login($user) {
        ...
        if(!isset($_SESSION['wid'])) {
            $query = mysqli_query($database->connection,'SELECT * FROM `' . TB_PREFIX . 'vdata` WHERE `owner` = ' . $database->getUserField($_SESSION['username'], "id", 1) . ' LIMIT 1');
