...
class Technology {
	...
	public function getUnitList() {
		...
		for($i=1;$i<=50;$i++) {
			if($unitcheck['u'.$i] >= "40000000000" && LIMIT_TROOPS) {
				mysqli_query($database->connection,"UPDATE ".TB_PREFIX."units set u".$i." = '0' where vref = $village->wid");
			}
		}
	...
	public function maxUnit($unit,$great=false) {
		...
		$unitarray = $$unit;
		$res = array();
		$res = mysqli_fetch_assoc(mysqli_query($database->connection,"SELECT maxstore, maxcrop, wood, clay, iron, crop FROM ".TB_PREFIX."vdata WHERE wref = ".$village->wid)) or die(mysqli_error($database->connection));
