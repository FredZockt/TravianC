...
$sql = mysqli_query("SELECT * FROM ".TB_PREFIX."users WHERE username LIKE 'Natars'");
...
if(count($result) > 1) ## Natar Account Exists
{
	...
	function Artefact($uid, $type, $size, $art_name, $village_name, $desc, $effect, $img)
	{
       	...
       	$database->addTech($wid);
       	$database->addABTech($wid);
        mysqli_query($database->connection, "UPDATE ".TB_PREFIX."vdata SET pop  = 163 WHERE wref = $wid");
