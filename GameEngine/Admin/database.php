...
class adm_DB {
  ...
  function buildingPOP($f,$lvl){
    ...
    $status = 0;
    if($status == 0){
	mysqli_query($this->connection, "Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Added new village <b><a href=\'admin.php?p=village&did=$wid\'>$wid</a></b> to user <b><a href=\'admin.php?p=player&uid=$uid\'>$uid</a></b>',".time().")");
  ...
  function CheckPass($password,$uid){       
	...
	mysqli_query($this->connection, "Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Deleted village <b>$wref</b>',".time().")");
	...
	function DelBan($uid,$id){
	 global $database;
	$name = $database->getUserField($uid,"username",0);
	mysqli_query($this->connection, "Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Unbanned user <a href=\'admin.php?p=player&uid=$uid\'>$name</a>',".time().")");
  ...
  function AddBan($uid,$end,$reason){
    global $database;
	$name = $database->getUserField($uid,"username",0);
	mysqli_query($this->connection, "Insert into ".TB_PREFIX."admin_log values (0,".$_SESSION['id'].",'Banned user <a href=\'admin.php?p=player&uid=$uid\'>$name</a>',".time().")");
  ...
  function Del_banned(){
	...
	***************************/
	function query($query) {
		return mysqli_query($this->connection, $query, $this->connection);
	}
