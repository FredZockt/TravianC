<?php
	...
	private $allNotice = array();
			...
			private function removeMessage($post) {
				...
				for($i = 1; $i <= 10; $i++) {
					if(isset($post['n' . $i])) {
					$message1 = mysqli_query($database->connection,"SELECT * FROM " . TB_PREFIX . "mdata where id = ".$post['n' . $i]."");
			...
			private function sendAMessage($topic,$text) {
				global $session,$database;
				$allmembersQ = mysqli_query($database->connection,"SELECT id FROM ".TB_PREFIX."users WHERE alliance='".$session->alliance."'");
				$userally = $database->getUserField($session->uid,"alliance",0);
				$permission=mysqli_fetch_array(mysqli_query($database->connection,"SELECT opt7 FROM ".TB_PREFIX."ali_permission WHERE uid='".$session->uid."'"));
