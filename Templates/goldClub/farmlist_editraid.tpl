...
<?php
$sql = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."farmlist WHERE owner = $session->uid ORDER BY name ASC");
...
$raidlist = mysqli_fetch_array(mysqli_query($database->connection, $select_raidlist));
$getwref = "SELECT * FROM ".TB_PREFIX."raidlist WHERE id = ".$raidlist['lid']."";
$arraywref = $database->query_return($getwref);
	echo '<option value="">Select village</option>';
if(mysqli_num_rows(mysqli_query($database->connection, $getwref)) != 0){
