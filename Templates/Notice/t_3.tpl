...
$sql = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE uid = $session->uid and archive = 0 $limit2 and del = 0 ORDER BY time DESC");
...
$sql2 = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE uid = $session->uid and archive=0 $limit2 and del = 0 ORDER BY time DESC $limit");
$paginationDisplay = "";
