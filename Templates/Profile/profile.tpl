...
$prefix = "".TB_PREFIX."vdata";
	$sql = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE owner = $session->uid ORDER BY pop DESC");
    $name = 0;
...
$sql2 = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE owner = ".$session->uid." AND conqured = ".$row['wref']."");
...
break;
    ...
    $sql2 = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE id = ".$row['wref']."");
