...
if($_POST) {
    ...
    foreach($links as $link) {
        ...
        if(trim($link['nr']) != '' AND trim($link['linkname']) != '' AND trim($link['linkziel']) != '' AND trim($link['id']) == '') {
            ...
            $query = mysqli_query($database->connection, 'INSERT INTO `' . TB_PREFIX . 'links` (`userid`, `name`, `url`, `pos`) VALUES (' . $userid . ', \'' . $link['linkname'] . '\', \'' . $link['linkziel'] . '\', ' . $link['nr'] . ')');
        } elseif(trim($link['nr']) != '' AND trim($link['linkname']) != '' AND trim($link['linkziel']) != '' AND trim($link['id']) != '') {
            // Update link
            $query = mysqli_query($database->connection, 'SELECT * FROM `' . TB_PREFIX . 'links` WHERE `id` = ' . $link['id']);
            $data = mysqli_fetch_assoc($query);
            
            // May the user update this entry?
            if($data['userid'] == $session->uid) {
                $query2 = mysqli_query($database->connection, 'UPDATE `' . TB_PREFIX . 'links` SET `name` = \'' . $link['linkname'] . '\', `url` = \'' . $link['linkziel'] . '\', `pos` = ' . $link['nr'] . ' WHERE `id` = ' . $link['id']);
            }
        } elseif(trim($link['nr']) == '' AND trim($link['linkname']) == '' AND trim($link['linkziel']) == '' AND trim($link['id']) != '') {
            // Delete entry
            $query = mysqli_query($database->connection, 'SELECT * FROM `' . TB_PREFIX . 'links` WHERE `id` = ' . $link['id']);
            ...
            if($data['userid'] == $session->uid) {
                $query2 = mysqli_query($database->connection, 'DELETE FROM `' . TB_PREFIX . 'links` WHERE `id` = ' . $link['id']);
            }
        }
    }
...
// Fetch all links
$query = mysqli_query($database->connection, 'SELECT * FROM `' . TB_PREFIX . 'links` WHERE `userid` = ' . $session->uid . ' ORDER BY `pos` ASC') or die(mysqli_error());
$links = array();
