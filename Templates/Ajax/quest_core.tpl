...
if (isset($qact)){
 switch($qact) {
	...
	$_SESSION['qst']= 3;
    $today = date("mdHi");
	mysqli_query($database->connection, "UPDATE ".TB_PREFIX."users set plus = ('".mktime(date("H"),date("i"), date("s"),date("m") , date("d"), date("Y"))."')+3600*24 where `id`='".$session->uid."'") or die(mysqli_error($database->connection));
	break;
