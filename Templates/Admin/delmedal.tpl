...
$sql = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."medal");
...
<table id="profile">
	...
	<tbody>
		<?php
			...
			$sql = mysqli_query($database->connection, "SELECT week FROM ".TB_PREFIX."medal ORDER BY week DESC LIMIT 1");
	...
	<tbody>
		<?php
			for($j = 0; $j<$week; $j++)
			{
				...
				$sql = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."medal WHERE week = $newweek");
				$tot = mysqli_num_rows($sql);
