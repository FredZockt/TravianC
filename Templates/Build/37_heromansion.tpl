...
if($_GET['gid']==37 && isset($_GET['del'])){
    ...
    mysqli_query($database->connection, "UPDATE ".TB_PREFIX."wdata SET occupied = 0 WHERE id = ".$_GET['del']."");
}
...
$sql = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE owner = $session->uid AND conqured = $village->wid ORDER BY lastupdated ASC");
...
<table id="oasesSurround" cellpadding="1" cellspacing="1">
    ...
    <tbody>
<?php
    $getoasis = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."wdata WHERE oasistype > 0");
