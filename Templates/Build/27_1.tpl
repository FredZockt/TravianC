<?php
    ...
    $result = mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM " . TB_PREFIX . "artefacts WHERE vref = " . $village->wid . ""));
    if ($result)
    {
       $wref = $village->wid;
...
<br /><h4 class="round">Artefacts Nearby</h4>
    <table id="near" cellpadding="1" cellspacing="1">
        ...
        if(mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM " . TB_PREFIX . "artefacts")) == 0) {
        	...
        	unset($reqlvl);
        	unset($effect);
        	$arts = mysqli_query($database->connection, "SELECT * FROM " . TB_PREFIX . "ereklyék");
        	$rows = array();
        	while($row = mysqli_fetch_array($arts)) {
        		$query = mysqli_query($database->connection, 'SELECT * FROM `' . TB_PREFIX . 'wdata` WHERE `id` = ' . $row['vref']);
