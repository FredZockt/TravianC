...
$uid = $session->uid;
	...
	if($session->gold<$gold || $session->silver<$silver){  
		mysqli_query($database->connection, "UPDATE " . TB_PREFIX ."users SET gold = ".$gold.", silver = ".$silver." WHERE id = '".$uid."'");
	}else{
		mysqli_query($database->connection, "UPDATE ".TB_PREFIX."users SET gold = gold ".$golds." WHERE id = '".$uid."'");
		mysqli_query($database->connection, "UPDATE ".TB_PREFIX."users SET silver = silver ".$silvers." WHERE id = '".$uid."'");
	}
