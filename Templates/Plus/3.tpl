<?php
    ...
    $MyGold = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."users WHERE `id`='".$session->uid."'") or die(mysqli_error($database->connection));
    $golds = mysqli_fetch_array($MyGold);

if($golds['b1'] <= time()) {
mysqli_query($database->connection, "UPDATE ".TB_PREFIX."users set b1 = '0' where `id`='".$session->uid."'") or die(mysqli_error($database->connection));
}

if($golds['b2'] <= time()) {
mysqli_query($database->connection, "UPDATE ".TB_PREFIX."users set b2 = '0' where `id`='".$session->uid."'") or die(mysqli_error($database->connection));
}
if($golds['b3'] <= time()) {
mysqli_query($database->connection, "UPDATE ".TB_PREFIX."users set b3 = '0' where `id`='".$session->uid."'") or die(mysqli_error($database->connection));
}

if($golds['b4'] <= time()) {
mysqli_query($database->connection, "UPDATE ".TB_PREFIX."users set b4 = '0' where `id`='".$session->uid."'") or die(mysqli_error($database->connection));
}
...
$date2=strtotime("NOW"); 
	...
	if ($datetimep == 0) {
		...
		mysqli_query($database->connection, "UPDATE ".TB_PREFIX."users set plus = '0' where `id`='".$session->uid."'") or die(mysqli_error($database->connection));
 	} else {
