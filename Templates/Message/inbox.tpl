...
$sql = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE target = $session->uid AND archived = 0 AND deltarget = 0 ORDER BY time DESC");
...
$sql2 = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE target = $session->uid AND archived = 0 AND deltarget = 0 ORDER BY time DESC $limit");
$paginationDisplay = "";
