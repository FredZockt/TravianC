...
<div class="option"><a href="karte.php?x=<?php echo $basearray['x']; ?>&y=<?php echo $basearray['y']; ?>" class="a arrow">Centre map</a></div>
 ...
 <?php 
    ...
    <div class="option">
          <?php 
          $query1 = mysqli_query($database->connection, 'SELECT * FROM `' . TB_PREFIX . 'vdata` WHERE `wref` = '.$d.'');
...
$result = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."ndata WHERE $limit AND ally = ".$session->alliance." AND (ally > 0  or uid = ".$session->uid.") AND toWref = ".$d." ORDER BY time DESC Limit 5");
