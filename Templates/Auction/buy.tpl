...
$sql = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE finish = 0 and owner != $session->uid $mmm ORDER BY time ASC");
...
$sql2 = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE finish = 0 and owner != $session->uid $mmm ORDER BY time ASC $limit");
