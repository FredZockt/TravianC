...
$sql = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE finish = 0 AND owner = $session->uid ORDER BY time ASC");
$query = mysqli_num_rows($sql); // Get the number of items already in auction
...
$sql2 = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE proc = 0 AND uid = $session->uid");
...
$sql = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE finish = 1 and owner = $session->uid ORDER BY time ASC");
...
$sql2 = mysqli_query($database->connection, "SELECT * FROM $prefix WHERE finish = 1 and owner = $session->uid ORDER BY time DESC $limit");
