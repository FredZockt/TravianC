...
$q = "
	...
	$result = mysqli_query($database->connection,$q) or die(mysqli_error($database->connection));
	while($row = mysqli_fetch_assoc($result))
	{
		$attacker[] = $row;
	}
	...
	$sql = mysqli_query($database->connection,"SELECT vref FROM ".TB_PREFIX."fdata WHERE f99 = '100' and f99t = '40'");
	$vref = mysqli_result($sql, 0);

	$sql = mysqli_query($database->connection,"SELECT name FROM ".TB_PREFIX."vdata WHERE wref = '$vref'")or die(mysqli_error($database->connection));
	$winningvillagename = mysqli_result($sql, 0);

	$sql = mysqli_query($database->connection,"SELECT owner FROM ".TB_PREFIX."vdata WHERE wref = '$vref'")or die(mysqli_error($database->connection));
	$owner = mysqli_result($sql, 0);

	$sql = mysqli_query($database->connection,"SELECT username FROM ".TB_PREFIX."users WHERE id = '$owner'")or die(mysqli_error($database->connection));
	$username = mysqli_result($sql, 0);

	$sql = mysqli_query($database->connection,"SELECT alliance FROM ".TB_PREFIX."users WHERE id = '$owner'")or die(mysqli_error($database->connection));
	$allianceid = mysqli_result($sql, 0);

	$sql = mysqli_query($database->connection,"SELECT name, tag FROM ".TB_PREFIX."alidata WHERE id = '$allianceid'")or die(mysqli_error($database->connection));
	$winningalliance = mysqli_result($sql, 0);

	$sql = mysqli_query($database->connection,"SELECT tag FROM ".TB_PREFIX."alidata WHERE id = '$allianceid'")or die(mysqli_error($database->connection));
