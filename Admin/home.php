...
<div id="div_2" style="display:none;">
    ...
    $sql = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."admin_log ORDER BY id DESC LIMIT 10");
    $query = count($sql);
        if($query>0){
            while($row = ifetch_array($sql)){
                $admid = $row['user'];
...
<div id="div_3" style="display:none;">
	<?php
    $tribe1 = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."users WHERE tribe = 1 and id>3");
    $tribe2 = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."users WHERE tribe = 2 and id>3");
    $tribe3 = mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."users WHERE tribe = 3 and id>3");
    $tribes = array( mysqli_num_rows($tribe1), mysqli_num_rows($tribe2), mysqli_num_rows($tribe3));
    $users =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."users WHERE id>3"));
    $actives =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."active"));
    $onlines =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."users WHERE ".time()." - timestamp < 300"));
    $banned =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."users WHERE access = 0"));
    $villages =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."vdata"));
    $alliances =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."alidata"));
    $adventures =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."adventure"));
    $auctions =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."auction WHERE finish = 0"));
    $notices =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."ndata"));
    $movements =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."movement WHERE proc = 0"));
    $allvillages =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."wdata WHERE oasistype = 0"));
    $alloasis =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."wdata WHERE fieldtype = 0"));
    $occoasis =  mysqli_num_rows(mysqli_query($database->connection, "SELECT * FROM ".TB_PREFIX."wdata WHERE fieldtype = 0 and occupied!=0"));
    ?><br />
