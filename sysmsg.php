...
if (mysqli_num_rows(mysqli_query($database->connection,"SELECT id FROM ".TB_PREFIX."users WHERE access = 9 AND id = ".$session->uid)) != '1') die("Hacking attempt!");
...
if(isset($_GET['del'])){
			...
			for ($i=0; $row=mysqli_fetch_row($result); $i++) {
					...
					$updateattquery = mysqli_query($database->connection,"UPDATE ".TB_PREFIX."users SET ok = '0' WHERE id = '".$row[0]."'")
	...
	if ($_POST['confirm'] == 'Yes'){
		...
		fwrite($fh, $text);
			...
			for ($i=0; $row=mysqli_fetch_row($result); $i++) {
					...
					$updateattquery = mysqli_query($database->connection,"UPDATE ".TB_PREFIX."users SET ok = '1' WHERE id = '".$row[0]."'")
